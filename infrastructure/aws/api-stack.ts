...
export class ApiStack extends cdk.Stack {
	constructor(scope: Construct, id: string, props: ApiStackProps) {
		...
		const taskDefinition = new ecs.FargateTaskDefinition(this, `TaskDef-${environment}`, {
				cpu,
				memoryLimitMiB,
		});

		taskDefinition.addContainer(`AppContainer-${environment}`, {
			image: ecs.ContainerImage.fromRegistry(`${this.account}.dkr.ecr.${this.region}.amazonaws.com/${ecrRepoName}:latest`),
			memoryLimitMiB,
		...
		new cdk.CfnOutput(this, `LoadBalancerDNS-${environment}`, { value: lb.loadBalancerDnsName });
	}
}